...
def main():
    ...
    data, proj, geotrans, row, col = read_tif(Target_path)
    month_list = [5, 6, 7, 8]
    for i in range(2013,2015,1):
        for j in month_list:
            print(CANet_path + '\\%d' % i + '*_'+'%.2d' % j +'*_*.tif')
            Year_month_list = sorted(glob.glob(CANet_path + '\\%d' % i + '*_'+'%.2d' % j +'*_*.tif'))
            if len(Year_month_list) == 0:
                print('List', i, '_', j,  'is empty')
            else:
                np_zeros = np.zeros((2488, 2370, 1), np.float16)
                for day_num in range(len(Year_month_list)):
                    Tar_data, Tar_proj, Tar_geotrans, Tar_row, Tar_col = read_tif(Year_month_list[day_num])
                ...
                geotrans_save = (geotrans[0],0.04,0,geotrans[3],0,-0.04)
                write_tif(Output_folder + '\\' + str(i) + '_' + str(j) + '.tif', data, geotrans_save, proj)
                del np_zeros,data
